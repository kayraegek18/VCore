...
final class VParticleTypes {
    static final Class<?> BLOCK_DATA_CLASS = VReflection.optionalClass("org.bukkit.block.data.BlockData").orElse(null);
    static final Class<?> DUST_OPTIONS_CLASS = VReflection.optionalClass("org.bukkit.Particle$DustOptions").orElse(null);
    static final Class<?> DUST_TRANSITION_CLASS = VReflection.optionalClass("org.bukkit.Particle$DustTransition").orElse(null);
    static final Map<Class<?>, Class<?>> DATA_ADAPTERS = new HashMap<>();
    static final boolean LEGACY = !VReflection.optionalClass("org.bukkit.Particle").isPresent();
    static final boolean MODERN = BLOCK_DATA_CLASS != null && Particle.BLOCK_CRACK.getDataType() == BLOCK_DATA_CLASS;
    static final VParticleData.DustOptions DEFAULT_DUST_OPTIONS = VParticleData.createDustOptions(Color.RED, 1);

    static {
        DATA_ADAPTERS.put(MaterialData.class, VParticleData.BlockData.class);
        DATA_ADAPTERS.put(Color.class, VParticleData.DustOptions.class);
        if (BLOCK_DATA_CLASS != null) {
            DATA_ADAPTERS.put(BLOCK_DATA_CLASS, VParticleData.BlockData.class);
        }
        if (DUST_OPTIONS_CLASS != null) {
            DATA_ADAPTERS.put(DUST_OPTIONS_CLASS, VParticleData.DustOptions.class);
        }
        if (DUST_TRANSITION_CLASS != null) {
            DATA_ADAPTERS.put(DUST_TRANSITION_CLASS, VParticleData.DustTransition.class);
        }
    }

    private VParticleTypes() {
    ...
    static VParticleType of(String name) {
        ...
        try {
            if (LEGACY) {
                return VLegacyParticleType.of(name.toUpperCase(Locale.ROOT));
            }
    ...
    static final class DefaultParticleType implements VParticleType {
        ...
        private Object mapData(Object data) {
            ...
            if (data instanceof VParticleData.AbstractParticleData) {
                data = ((VParticleData.AbstractParticleData) data).data;
            }
